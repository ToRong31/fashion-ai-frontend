  ...
  return (
    <div className="flex items-center gap-4 py-4 border-b border-border">
      <div className="w-16 h-20 rounded bg-surface flex-shrink-0 flex items-center justify-center">
        <span className="text-text-secondary text-xs uppercase">{item.size ?? '—'}</span>
      </div>
      <div className="flex-1 min-w-0">
        <h4 className="text-text-primary text-sm font-medium truncate">{item.product_name}</h4>
        {item.size && (
          <p className="text-text-secondary text-xs mt-0.5">Size: {item.size}</p>
        )}
        <p className="text-gold text-sm mt-0.5">${item.price.toFixed(2)}</p>
      </div>
